...
def _audit_portfolio_risk(today_str: str, now: datetime):
...
def _scan_watchlist_opportunities(today_str: str, now: datetime):
    ...
    for item in watchlist:
        ...
        try:
            ...
            if target_buy > 0 and curr_p <= target_buy:
                buy_triggered = True
                buy_reason = f"Thị giá ({curr_p}k) đã chạm/về dưới vùng giá chờ mua ({target_buy}k)!"
                ...
                buy_triggered = True
                buy_reason = f"RSI({rsi:.1f}) rơi vào vùng QUÁ BÁN sâu (< 32)!"
...
def check_realtime_risk():
    ...
    _audit_portfolio_risk(today_str, now)
    _scan_watchlist_opportunities(today_str, now)
